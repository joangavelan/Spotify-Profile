...
const Body = ({ playlist }) => {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if(playlistTracksRef.current && recommendedTracksRef.current && !playlistTracksRef.current.contains(event.target) && !recommendedTracksRef.current.contains(event.target) && !event.target.matches('.play-button')) {
